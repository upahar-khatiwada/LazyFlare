...
pub fn server() {
    ...
    loop {
        ...
        let response: String = create_response(filled_buf);
        let response_bytes = hex::decode(&response).expect("Failed to decode response hex");
        socket.send_to(&response_bytes, src_addr).expect("Failed to send response");
    }
}